...
TThostFtdcBrokerIDType brokerid="----";		// 应用单元
TThostFtdcUserIDType userId="----";		// 投资者代码
TThostFtdcAppIDType  appID="----";
TThostFtdcPasswordType	passwd="----";
TThostFtdcAuthCodeType AuthCode = "----";
TThostFtdcUserProductIDType UserProductInfo = "----";
...
{
	cerr<<" 连接交易前置...成功 "<< tradeFront <<endl;
	static const char *version = pUserApi->GetApiVersion();
	        ...
	        TThostFtdcUserIDType	vUserId,	TThostFtdcPasswordType	vPasswd)
{
	...
	strcpy(req.BrokerID, vAppId); strcpy(brokerid, vAppId); 
...
void CtpTraderSpi::ReqOrderAction(TThostFtdcSequenceNoType orderSeq)
{
  ...
  if(!found){cerr<<" 请求 | 报单不存在."<<endl; return;} 
	...
	memset(&req, 0, sizeof(req));
	strcpy(req.BrokerID, brokerid);   //经纪公司代码	
	strcpy(req.InvestorID, userId); //投资者代码
...
void CtpTraderSpi::ReqAuth()
{
	...
	strcpy(req.AppID, "client_jsctp_1.0.0");
	strcpy(req.UserID, "10102289");
	strcpy(req.AuthCode, "7NYG7TZIOKBJ7FAH");*/
	//strcpy(req.UserProductInfo, "client_jsctp_1.0.0");
	 
	int ret = pUserApi->ReqAuthenticate(&req, ++requestId);
	cerr << " 请求 | 发送认证... \nappID="  << req.AppID
		<< " brokerid=" << req.BrokerID
		<< " userId=" << req.UserID
		<< " AuthCode=" << req.AuthCode
		//<< " UserProductInfo=" << req.UserProductInfo
	<<"\n" 
	<< ((ret == 0) ? "成功" : "失败") << endl;
}

void CtpTraderSpi::ReqSubmit()
{
	CThostFtdcUserSystemInfoField req;
	memset(&req, 0, sizeof(req));
	strcpy(req.BrokerID, brokerid);
	strcpy(req.ClientAppID, appID);
	char buf[512] = { 0 };
	int len = 0;
	//xpk remove
	//int r = CTP_GetSystemInfo(buf, len);
	//memcpy(req.ClientSystemInfo, buf, len);
	//req.ClientSystemInfoLen = len;
	strcpy(req.UserID, userId);
	int ret = pUserApi->SubmitUserSystemInfo(&req);
	cerr << " 请求 | 上报用户终端信息..." << ((ret == 0) ? "成功" : "失败") << endl;
}
...
void CtpTraderSpi::OnRspAuthenticate(CThostFtdcRspAuthenticateField *pRspAuthenticateField, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	//if (bIsLast) SetEvent(g_hEvent);
	if (pRspAuthenticateField && pRspAuthenticateField->AppType == THOST_FTDC_APP_TYPE_InvestorRelay)
	{
		CThostFtdcUserSystemInfoField req;
		memset(&req, 0, sizeof(req));
		strcpy(req.BrokerID, brokerid);
		...
		//req.ClientSystemInfoLen = len;
		strcpy(req.UserID, userId);
		/*strcpy(req.ClientPublicIP, "122.224.196.124");
		req.ClientIPPort = 12350;
		strcpy(req.ClientLoginTime, "09:39:00");*/
		int ret = pUserApi->RegisterUserSystemInfo(&req);
		cerr << " 请求 | 注册用户终端信息..." << ((ret == 0) ? "成功" : "失败") << endl;
	}
